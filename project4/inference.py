  ...
  def elapseTime(self, gameState):
    ...
    for p in self.legalPositions:
        ...
        for newp in newPosDist:
            allPossible[newp]+=newPosDist[newp]*self.beliefs[p]
    allPossible.normalize()
...
class ParticleFilter(InferenceModule):
  ...
  def __init__(self, ghostAgent, numParticles=500):
  ...
  def initializeUniformly(self, gameState):
    ...
    for i in range(1,self.numParticles+1):
        list.append(random.choice(self.legalPositions))
        weight.append(1)
    self.myNodes= list
    self.weights=weight
    self.weights=[float(i)/sum(self.weights) for i in self.weights]
    #self.allPossible=util.Counter()
    #for p in self.legalPositions:
      #  self.allPossible[p]=1
    #self.allPossible.normalize()
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance==None:
        for i in range(0,self.numParticles):
            self.myNodes[i]=self.getJailPosition()
            self.weights[i]=1
        self.weights=[float(i)/sum(self.weights) for i in self.weights]
        return
...
#        self.weights[i]=0
    ...
    for i in range(0,self.numParticles):
        p=self.myNodes[i]
        trueDistance = util.manhattanDistance(p, pacmanPosition)
        #self.allPossible[p]*=emissionModel[trueDistance]
        self.weights[i] = emissionModel[trueDistance]*(self.getBeliefDistribution()[p])
    ...
    distri = util.Counter()
    for i in range(0,self.numParticles):
        distri[self.myNodes[i]]=self.weights[i]
    for i in range(0,self.numParticles):
        self.myNodes[i]=util.sample(distri)
        self.weights[i]=1
  ...
  def elapseTime(self, gameState):
    ...
    for i in range(0,self.numParticles):
        ...
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.myNodes[i]))
        
        self.myNodes[i]=util.sample(newPosDist)
        p=self.myNodes[i]
        #self.allPossible[p]*=newPosDist[self.myNodes[i]]
    self.weights=[float(i)/sum(self.weights) for i in self.weights]
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
        ...
        for j in range(self.numGhosts):
            theList.append(random.choice(self.legalPositions))
        self.particles.append(tuple(theList))
  ...
  def observeState(self, gameState):
    ...
    for i in range(self.numParticles):
        self.weights[i]=0
    #self.weights=[float(i)/sum(self.weights) for i in self.weights]
    mybelief=self.getBeliefDistribution()
    for i in range(self.numParticles):
        weight=1
        for j in range(self.numGhosts):
            if self.particles[i][j]==self.getJailPosition(j):
                continue
            p=self.particles[i][j]
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            weight*=emissionModels[j][trueDistance]
            
        self.weights[i] += weight*mybelief[self.particles[i]]
    ...
    dist=util.Counter()
    for i in range(self.numParticles):
        dist[self.particles[i]]=self.weights[i]
    for i in range(self.numParticles):
